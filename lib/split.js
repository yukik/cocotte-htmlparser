...
var getLineColumn = require('./utils/getLineColumn');
...
function split(html, e) {
  ...
  while(m = TAG.exec(html)) {
    ...
    if (index !== lastIndex) {
      ...
      var textNode = createTextNode(lastIndex, getPos, text);
      nodes.push(textNode);
    }
 ...
 */
function createTextNode(index, getPos, original) {
  var result = analyzeWhiteSpace(original);
  var pos = getPos(index + result.index);
  var textNode = {
    line: pos.line,
    column: pos.column,
...
function setAttributes (node, attributes, pos, e) {
  ...
  if (tag && attrs) {
    ...
    if (disabled.length) {
      var message = [M.DISABLED_ATTRIBUTE, node.tagName, disabled.join(',')];
      e(pos, message, 'notice');
    }
  }
}
 ...
 * @param  {Function} getPos
 */
function checkDoctype(nodes, e, getPos) {
  var first = nodes[0];
  ...
  if (m) {
    ...
    if (text) {
      ...
      var textNode = createTextNode(textIndex, getPos, text);
